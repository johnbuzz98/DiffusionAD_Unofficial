...
def training(
    ...
    # set train mode
    denoising_model, segmentation_model = model
    ...
    while train_mode:
        ...
        for inputs, masks, targets in trainloader:
            ...
            pred_noise, noise = denoising_model.loss(inputs)
            mse_loss = mse_criterion(pred_noise, noise)

            # denoise
            noised_result = denoising_model.q_sample(inputs, denoising_model.t)
            denoised_input = denoising_model.p_sample(noised_result, denoising_model.t)

            joined_in = torch.cat((inputs, denoised_input), dim=1)
            out_mask = segmentation_model(joined_in)
    ...
    with torch.no_grad():
        for idx, (inputs, masks, targets) in enumerate(dataloader):
            ...
            time_stamp = torch.randint(
                0,
                denoising_model.n_steps,
                (inputs.size(0),),
                device=device,
                dtype=torch.long,
            )
            noised_result = denoising_model.q_sample(inputs, time_stamp)
            denoised_input = denoising_model.p_sample(noised_result, time_stamp)
